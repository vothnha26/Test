-- =============================================
-- Database QuanLyNhanSu - VERSION FIXED
-- Đã sửa tất cả lỗi logic tính lương, bảo mật và performance
-- =============================================
...
CREATE TABLE [dbo].[Luong](
    ...
    [NgayTinh] [datetime2] DEFAULT GETDATE(),
...
GO
ALTER TABLE [dbo].[UserSessions] ADD FOREIGN KEY([MaNV]) REFERENCES [dbo].[NhanSu] ([MaNV])
GO
...
CREATE INDEX IX_LoginAttempts_TenDangNhap_Timestamp ON LoginAttempts (TenDangNhap, Timestamp);
CREATE INDEX IX_UserSessions_MaNV_IsActive ON UserSessions (MaNV, IsActive);

-- =============================================
-- 6. TẠO FUNCTIONS (ĐÃ SỬA LỖI)
-- =============================================

-- Function kiểm tra hợp đồng có thể xóa (đã tối ưu)
...
-- Function kiểm tra trùng lịch nghỉ (đã tối ưu)
...
-- Function lấy lương cơ bản hiện hành (đã cải thiện)
...
-- Function tính số ngày công chuẩn (đã linh hoạt hơn)
